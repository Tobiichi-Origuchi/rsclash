...
impl MihomoApi for MihomoClient {
  ...
  async fn flush_fake_ip_cache(&self) -> Result<()> {
    self
      .request_empty(RequestSpec::new(Method::POST, "/cache/fakeip/flush"))
      .await
  }

  async fn flush_dns_cache(&self) -> Result<()> {
    self
      .request_empty(RequestSpec::new(Method::POST, "/cache/dns/flush"))
      .await
  }

  async fn connections(&self) -> Result<Connections> {
    self
      .get_json("connections", "/connections".to_string())
      .await
  }

  async fn close_all_connections(&self) -> Result<()> {
    self
      .request_empty(RequestSpec::new(Method::DELETE, "/connections"))
      .await
  }

  async fn close_connection(&self, connection_id: &str) -> Result<()> {
    self
      .request_empty(RequestSpec::new(
        Method::DELETE,
  ...
  async fn group(&self, name: &str) -> Result<Proxy> {
    self
      .get_json("group", format!("/group/{}", encode(name)))
      .await
  }
  ...
  async fn proxy_providers(&self) -> Result<ProxyProviders> {
    self
      .get_json("proxy providers", "/providers/proxies".to_string())
      .await
  }
  ...
  async fn healthcheck_proxy_provider(&self, name: &str) -> Result<()> {
    self
      .request_empty(RequestSpec::get(format!(
  ...
  async fn proxy(&self, name: &str) -> Result<Proxy> {
    self
      .get_json("proxy", format!("/proxies/{}", encode(name)))
      .await
  }
  ...
  async fn rule_providers(&self) -> Result<RuleProviders> {
    self
      .get_json("rule providers", "/providers/rules".to_string())
      .await
  }
  ...
  async fn patch_base_config(&self, patch: Value) -> Result<()> {
    self
      .request_empty(RequestSpec::new(Method::PATCH, "/configs").with_json(&patch)?)
      .await
  }
  ...
  async fn restart(&self) -> Result<()> {
    self
      .request_empty(RequestSpec::new(Method::POST, "/restart"))
      .await
  }
