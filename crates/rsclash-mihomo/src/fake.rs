...
impl MihomoApi for FakeMihomoApi {
    ...
    async fn group(&self, name: &str) -> Result<Proxy> {
        self.record(MihomoCall::Group(name.to_string()))?
            .state
            .groups
            .proxies
            .iter()
    ...
    async fn proxy_provider(&self, name: &str) -> Result<ProxyProvider> {
        self.record(MihomoCall::ProxyProvider(name.to_string()))?
            .state
            .proxy_providers
            .providers
            .get(name)
    ...
    async fn update_proxy_provider(&self, name: &str) -> Result<()> {
        self.record(MihomoCall::UpdateProxyProvider(name.to_string()))
            .map(|_| ())
    }

    async fn healthcheck_proxy_provider(&self, name: &str) -> Result<()> {
        self.record(MihomoCall::HealthcheckProxyProvider(name.to_string()))
            .map(|_| ())
    }
    ...
    async fn proxy(&self, name: &str) -> Result<Proxy> {
        self.record(MihomoCall::Proxy(name.to_string()))?
            .state
            .proxies
            .proxies
            .get(name)
    ...
    async fn update_rule_provider(&self, name: &str) -> Result<()> {
        self.record(MihomoCall::UpdateRuleProvider(name.to_string()))
            .map(|_| ())
    }
    ...
    async fn reload_config(&self, path: &str, force: bool) -> Result<()> {
        self.record(MihomoCall::ReloadConfig {
            path: path.to_string(),
            force,
        })
        .map(|_| ())
    ...
    async fn upgrade_core(&self, channel: CoreUpdaterChannel, force: bool) -> Result<()> {
        self.record(MihomoCall::UpgradeCore { channel, force })
            .map(|_| ())
    }
...
mod tests {
    ...
    async fn fake_records_mutations_and_updates_selected_proxy() {
        ...
        fake.select_proxy("GLOBAL", "Node A")
            .await
    ...
    async fn injected_failure_is_consumed_once() {
        let fake = FakeMihomoApi::default();
        fake.fail_next(Error::Fake("injected".to_string()))
