...
impl CvrImporter {
    ...
    pub fn import(&self) -> Result<CvrImportOutcome> {
        ...
        let receipt = ImportReceipt {
            version: 1,
            source_root: fs::canonicalize(&self.source_root).map_err(|source| {
                Error::io("canonicalize CVR source", &self.source_root, source)
            })?,
...
impl ImportBundle {
    fn load(source_root: &Path) -> Result<Self> {
        ...
        let metadata = fs::symlink_metadata(&profiles_root).map_err(|source| {
            Error::io("inspect CVR profiles directory", &profiles_root, source)
        })?;
        if metadata.file_type().is_symlink() || !metadata.is_dir() {
            return Err(Error::InvalidConfiguration(format!(
...
fn apply_import_writes(writes: &BTreeMap<PathBuf, Vec<u8>>, marker: &Path) -> Result<()> {
    ...
    let marker_content = writes.get(marker).ok_or_else(|| {
        Error::InvalidConfiguration("CVR import marker was not prepared".to_string())
    })?;
...
fn make_backup_read_only(path: &Path) -> Result<()> {
    #[cfg(unix)]
    {
        ...
        {
            let entry =
                entry.map_err(|source| Error::io("read import backup entry", path, source))?;
                ...
                .is_dir()
            {
                ...
                fs::set_permissions(&entry_path, fs::Permissions::from_mode(0o400)).map_err(
                    |source| Error::io("make import backup read-only", &entry_path, source),
                )?;
            }
        }
...
mod tests {
    ...
    fn imports_valid_cvr_data_once_without_linking_source() {
        ...
        assert_eq!(
            fs::read_to_string(report.backup_directory.join("verge.yaml"))
    ...
    fn rejects_symlinked_source_profile_without_modifying_target() {
        ...
        write_valid_source(&source.path);
        fs::remove_file(source.path.join("profiles/local.yaml"))
    ...
    fn failed_import_restores_every_target_file() {
        ...
        *super::INJECTED_MARKER_FAILURE
            .lock()
            .expect("failure injection should lock") =
            Some(target.path.join(".cvr-imported-v1.yaml"));
    ...
    impl TestDirectory {
        fn new(label: &str) -> Self {
            ...
            let path =
                std::env::temp_dir().join(format!("rsclash-{label}-{}-{id}", std::process::id()));
