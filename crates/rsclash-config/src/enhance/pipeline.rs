...
fn listener_is_disabled(field: &str, application: &ApplicationLayer) -> bool {
    match field {
        ...
        "redir-port" => {
            application.platform == TargetPlatform::Windows || !application.listeners.redir
        }
        "tproxy-port" => {
            application.platform != TargetPlatform::Linux || !application.listeners.tproxy
        }
        _ => false,
    }
}
