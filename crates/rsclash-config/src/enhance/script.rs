...
impl ScriptExecutor for BoaScriptExecutor {
  fn execute(
    ...
    thread::Builder::new()
      ...
      .map_err(|error| Error::ScriptExecution(format!("failed to start script worker: {error}")))?;
...
fn execute_inner(
  ...
  let config_json = serde_json::to_string(config)
    .map_err(|error| Error::ScriptExecution(format!("failed to encode config: {error}")))?;
  let profile_json = serde_json::to_string(profile_name)
    .map_err(|error| Error::ScriptExecution(format!("failed to encode profile name: {error}")))?;
  ...
  let value = context
    ...
    .map_err(|error| Error::ScriptExecution(format!("JavaScript evaluation failed: {error}")))?;
  if !value.is_string() {
  ...
  let output = serde_json::from_value::<Mapping>(value)
    .map_err(|error| Error::ScriptExecution(format!("invalid configuration output: {error}")))?;
  Ok(ScriptOutput {
