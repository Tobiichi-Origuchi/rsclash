...
pub(crate) fn recover_pending_transactions(root: &Path) -> Result<()> {
  ...
  for entry in
    fs::read_dir(root).map_err(|source| Error::io("read transaction directory", root, source))?
  {
...
fn sync_directory(path: &Path) -> Result<()> {
  #[cfg(unix)]
  {
    let directory = File::open(path).map_err(|source| Error::io("open directory", path, source))?;
    directory
      .sync_all()
...
mod tests {
  ...
  fn opening_store_recovers_an_unfinished_file_transaction() {
    ...
    let _unfinished =
      RollbackJournal::create(&directory.path, &snapshots).expect("journal should be durable");
  ...
  impl TestDirectory {
    fn new() -> Self {
      ...
      let path = std::env::temp_dir().join(format!("rsclash-journal-{}-{id}", std::process::id()));
